...
Nothing in it is reimplemented. The original ROM runs unmodified in Genesis Plus GX at 60Hz, so combat, dialogue, physics, item handling, puzzle state and room transitions all still happen in the 68000 code. What I replaced is the presentation layer. A bridge reads the emulator's work RAM every frame and streams the game state over a websocket, and a three.js renderer draws that state as real geometry, using tile and sprite art extracted from the ROM. The emulator's own video output gets thrown away; it is in there as a physics and logic server. Audio passes straight through, because there was no reason to touch it.
...
The bridge is a Python process holding the Genesis Plus GX libretro core through ctypes. It runs a frame, reads memory, sends messages, sleeps until the next 60Hz tick. The whole protocol fits in a docstring:
...
Two things about the bridge cost me real time.
...
Landstalker stores its rooms as a heightmap. Every cell carries a floor height, a set of collision flags, and a floor type. The game needs this because it is doing real 3D collision internally, on a 68000, in 1992. So the geometry I want is already in the cartridge, and the job is decoding it.
...
I did not work any of that out from raw bytes, and this is the part I would push hardest on if someone were starting a project like this. Landstalker has been reverse engineered for years by people who were very good at it. There is a full disassembly that reassembles byte-identical, a C++ format library, an editor, and two randomizers, one of which reads live RAM from a running US ROM and therefore corroborates memory addresses rather than cartridge ones. All five got vendored into a `refs/` directory and treated as the specification. The project spec I started from deliberately contained zero addresses, with a note attached: any offsets it supplied from memory would be unreliable, so derive every one from those sources or from live investigation, and write each into `docs/offsets.md` with a note on how it was confirmed. That file has a provenance column, and the entries that were never confirmed against a live emulator are still sitting there marked unverified. One of the format docs says outright that two flag bits are disputed between two reference implementations and that I do not know which is right.
...
So before the interesting work, I built three oracles.
...
Then there is a different kind of harness, which compares the emulator's own framebuffer against a pure-Python render of the same room from extracted assets, and reports a percentage. Getting that number to mean anything took more care than the comparison itself. It waits for the palette fade to finish and the camera words to stop moving before capturing, renders colours through the emulator's exact RGB565 expansion rather than a naive ramp, and masks out sprite pixels by walking the VDP sprite attribute table's link list, since entities are not part of a static room render. The number is 98.1% average background match across the reachable rooms, worst room 97.0%, with the residual being animated tile phase and a couple of places where the game edits its own tilemap at runtime. There is a rope bridge in the prologue that physically collapses, so the live tilemap differs from the cartridge there.
...
There is a counter-example. The animated tileset table has a length field, and I read it as bytes. It is VDP words, so every animation frame was half the size it should be. No oracle covered that table, so nothing failed. It took reading the game's own DMA routine and watching live VRAM to notice, which is a fair summary of what the whole project is like in the places where a ground truth does not exist.

## Drawing it

The projection comes straight out of the disassembly, from a routine that converts world coordinates to screen coordinates every frame:
...
Written continuously, the map is `screen_x = 16(x - y)` and `screen_y = 8(x + y) - 16z`. The view axis, the direction that projects to a single point, is exactly (1, 1, 1). Which means depth is just `x + y + z`.

That is the whole renderer, really. The geometry is built directly in screen space, with `x + y + z` written into the z coordinate, viewed through a plain orthographic camera. No rotation matrices, no camera rig. It is exact by construction, and every other trick in the project ends up reusing that one identity.

Textures are palette indices, not colours. Each room rasterises into a two-channel texture holding a 0-15 palette index and an emissive flag per pixel, and the fragment shader resolves the index against a 16-entry palette texture that gets re-uploaded from live colour RAM every frame. Which means fades, menu dimming, damage flashes and room transitions all come free and stay frame-accurate, because they are palette animations in the original and they are still palette animations here.
...
One nice side effect of not being the hardware: the original draws 320x224 because that is what it had. The 3D view has no VRAM limit, so the viewport fits the original playfield and then shows more of the room around it when the browser window is bigger.
...
The clipping stopped existing. Room geometry can no longer shave a sprite under any circumstances, because on the console it never could. The artists drew those rooms knowing that, and placed roofs and ledges and eaves to take advantage of it. My earlier attempts were fighting their art direction with a depth buffer.
...
The torches were the first thing that felt like cheating.

I wanted real point lights, but hand-placing them across 816 rooms was obviously not happening. Flames in this game are animated tiles, and a warm-coloured animated tile is in practice always fire, since the other animated tiles are water. So the renderer scans a room's block layers, picks out animated tiles whose average colour is strongly red-dominant, clusters adjacent cells, and emits one point light per cluster. No hand authoring, and it covers every room.

There is a wrinkle, and its solution is my favourite three lines in the project. A flame painted on a wall is at the wall's position, but a light source at that position sits inside the geometry. And because the view axis is (1, 1, 1), every point along that axis projects to the same screen pixel. So the light slides along the view axis until it finds passable floor, which puts it hovering just in front of the wall while its projection stays exactly on the painted flame.
...
Every one of these features defaults to off. With the neutral settings the shader arithmetic reduces to multiplying by one and adding zero, and exact mode forces them off entirely, so the pixel harness cannot drift while I am making things pretty. That is the only reason I still trust the 98.1% after four days of visual changes.

## The other half: the art, and the gate that could not see

Nothing in this section has an oracle, and it shows.

The idea was straightforward. Rasterise a room canvas from the extracted assets, pad it with black to the nearest aspect ratio the image API supports, send it image-to-image to Gemini at 4K with a prompt insisting the layout is authoritative, resize the result back to exactly 4x, crop the padding, and force the void area black again. Room variants share art, so deduplicating by rendered identity turns 816 rooms into 647 unique canvases. Then the renderer swaps the palette-index albedo for the generated image and keeps every other feature: lighting, shadows, ambient occlusion, the priority overlay, and the colour-RAM fade tracking, so HD rooms still fade correctly through doors.

It works. It looks, to my eye, better than I expected, and I went through six style candidates before settling on a Vanillaware-ish painted register.
...
The first gate was alignment. Take the generation, downscale it back to source resolution, run a Sobel filter on both, take the top decile of gradient magnitude as "strong edges", and measure the median distance from each source strong edge to the nearest generated strong edge. Require it under four pixels. Also FFT phase-correlate the two gradient fields and require a global shift of exactly zero. Three attempts, then mark the canvas rejected and fall back to pixel art.

That gate is good at what it measures. It catches the model reframing or rescaling the image, which it does surprisingly often, and two canvases have never passed it in any style because the model insists on shifting them.
...
So I built a second gate that tries to see meaning. For every cell in the game's own heightmap, project its top face into the canvas using the same projection the renderer uses, and sample a patch in both the source render and the generation. Blur each patch into an opponent-colour vector (luminance, red minus green, blue minus yellow) so brushwork averages out and only the material-level colour survives. Then the test: cells whose source colour matches each other, anywhere in the room, are the same material, so their generated colours should also match each other. Take each cell's deviation from the median of its own reference group. Faithful restyling leaves small scattered deviations. Semantic inversion leaves large, spatially connected, directionally coherent blobs.
...
It validates. All three broken canvases I had kept as backups reject. Forty-five of the forty-six previously accepted canvases pass, and the one that flags was independently known to be degraded.
...
There is more. A pit in a floor got painted as a pool of water; the gate rejected two attempts and accepted the third, because the pool was dark and my "not darkened" condition let it through. Conifer trees appeared inside a cave, and the water detector was structurally incapable of firing because the colour drift went the wrong way on every one of its axes. The two canvases that never pass sample one cell and twenty cells respectively, both below the thirty-cell minimum where the structural gate declares itself inconclusive and passes by default, so on the rooms most likely to fail it never runs at all.

The prompt is an archaeological record of the same process. It started at six clauses and grew one per disaster. There is a clause about trees keeping their exact count and species because trees were merging. There is a clause spelling out which part of an isometric interior is wall and which is floor, which exists because of the church. There is a clause demanding water stay saturated blue. And there is clause eight, which reads in part "A flat wall decoration in the source stays a flat wall decoration; never reinterpret one as a ladder or any other three-dimensional object", which exists because a test generation turned a wall banner into a ladder.

The character sprites went the same way, with a nice twist. All of a character's frames go into one grid and get regenerated in a single call, because that is what holds identity constant across poses. Alpha authority never leaves the original: the source silhouette, dilated two pixels, caps what can be opaque, so a generated sprite physically cannot leak outside an honest outline. I measured identity drift between adjacent walk frames and got a worst-case channel shift of 6 out of 255, which is excellent.

And in motion they shimmer. Hair tufts change shape, the scabbard drifts a few degrees, the dog's harness wobbles, and at 60Hz it reads as a faint boil. The metric I built measured colour consistency, which held up, and said nothing about shape consistency, which failed.
...
Numbers first, since that is what I would want.

Four days, three substantive sessions, roughly twenty background subagents, and 62 messages typed by me. The first one was 172 characters:
...
I want to be precise about that last number, because the flattering version of this story is that I set the constraints and then stepped back, and the transcripts do not support it.
...
But I never once defended those non-goals during the build, in 62 messages, and not because nothing came close. Click-to-move pathfinding and the step-up jump assist both sit right on the boundary between presentation and gameplay. The agent held the line itself, in the spec's own vocabulary, and wrote the reasoning into the commit messages. Two of my three session-opening prompts were handover documents the previous session's agent had written for me, which I pasted without editing. The single most technically detailed "human" message in the entire corpus is one of those, and I did not write a word of it. On day four I described my own project back to the agent to check I had it right, and got three corrections and two omissions in return.
